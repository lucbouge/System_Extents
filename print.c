...
static void print_header_for_file(unsigned i)
{
    ...
    for (hdr_line = 1; hdr_line <= 2; hdr_line++)
    {
        ...
        if (print_phys_addr)
            print_off_t_s(h("", "Physical", "Offset"));
        print_off_t_s(h("", "Length", ""));
        if (print_flags)
            fputs(h("", "  Flags", ""), stdout);
        putchar('\n');
    }
}
...
void print_extents_by_file(void)
{
    for (unsigned i = 0; i < nfiles; i++)
    {
        if (!no_headers)
            print_header_for_file(i);
        for (unsigned e = 0; e < info[i].n_exts; ++e)
        {
            ...
            print_extent(ext);
            if (print_flags)
                printf(" %s", flag_pr(ext->flags, false));
            putchar('\n');
        }
    }
}

void print_shared_extents_no_header(void)
{
...
void print_shared_extents(void)
{
    if (n_elems(shared) == total_self_shared)
        return;
    if (!no_headers)
    {
        if (!print_shared_only)
            puts("Shared: ");
...
void print_self_shared_extents(void)
{
    if (!no_headers)
    {
        if (!print_shared_only)
...
void print_unshared_extents(void)
{
    if (total_unshared == 0)
        return;
    if (!no_headers && !print_unshared_only)
        puts("Not Shared:");
    for (unsigned i = 0; i < nfiles; i++)
    {
        ...
        if (!is_empty(unsh))
        {
            ...
            ITER(unsh, sh_ext *, sh, {
                ...
                {
                    sep();
                    fputs(flag_pr(owner->flags, true), stdout);
                }
                putchar('\n');
            })
        }
...
void debug_print_extents(unsigned ei, extent *cur, list *owners)
{
    putchar('{');
    if (owners != NULL)
        ITER(owners, extent *, owner, printf("%d,", owner->info->argno))
    putchar('}');
    if (cur != NULL)
        print_extent(cur);
    putchar('!');
